...
/* options */\
#ytpc_options_popup {direction:ltr; position:absolute; top:10px; right:5px; width:235px; box-shadow:0px 0px 6px 2px gray; font-size:11px; color:black; background:white; padding:5px; border-radius:5px; z-index:10; user-select:none; -moz-user-select:none;}\
ytd-watch-metadata #ytpc_options_popup {top:0px !important;}\
...
init_pref("ytDef", "hd2160"); // Modified
init_pref("ytLoop", false);
init_pref("ytCine", false); // Modified
init_pref("ytStretch", false);
init_pref("ytHide", false);
init_pref("ytAnnot", true); // Modified
init_pref("ytAds", false);
init_pref("ytAdsExtra", false);
init_pref("ytFloat", false); // Modified
init_pref("ytFSmall", false); // Modified
...
function ytplayer_options() {
    ...
    new_checkbox("ytFloat", "Float Video", "span", groupFloat, null, function () { reset_float(); float(0); }, true, false);
...
function build_yt_control() {
        var parent = doc.getElementById("ytpc_title_container");
        if (parent) return;

        var pp = doc.getElementsByClassName("watch-active-metadata");
        if (pp.length == 0) return;

        parent = newNode("span", "ytpc_title_container", null, pp[0], 'first');
...
function cinema(start_count) {
	...
	insertStyle("\
		ytd-watch-flexy[theater]:not([float]) #player-theater-container {height:" + view_height + "px !important; max-height:" + view_height + "px !important; min-height:" + view_height + "px !important;}\
		ytd-watch-flexy[theater]:not([float]) .html5-main-video {width:" + width + "px !important; height:" + height + "px !important; left:" + left + "px !important; top:" + top + "px !important;margin-left:0px !important;}\
		", "ytpc_style_cinemode");
}
...
function skip_ads(start_count) {
    ...
    //adjust cinema after adds or fullscreen change by forcing resize event
    if (get_pref("ytCine")) {
...
win.addEventListener("resize", function () { reset_nochanges(); cinema(0); float(0); }, false);
win.addEventListener("scroll", function () { reset_nochanges(); cinema(0); float(0); }, false);
